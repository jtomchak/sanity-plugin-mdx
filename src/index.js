...
export default class MarkdownInput extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      !prevState.showUrlDialogFor &&
      this.state.showUrlDialogFor &&
      this._urlInput
    ) {
  ...
  handleSetWriteMode = () => {
    this.setState({ mode: "write", editorHeight: this.getEditorHeight() }, () =>
      this.focus()
    );
  };
  ...
  handleToggleMode = () => {
    ...
    const usePreview =
      typeof options.usePreview === "undefined" ? true : options.usePreview;
    if (!usePreview) {
      return;
    }
  ...
  render() {
    ...
    const MarkdownPreview = options.renderPreview || MDXPreview;
    // const MDXPreview = options.renderPreview || MDXPreview;
